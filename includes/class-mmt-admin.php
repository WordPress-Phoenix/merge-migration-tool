...
class MMT_Admin {
	 ...
	 * This definition excludes the wizard.
	 *
	 * @since 1.0.0
	 */
	...
	public function display_admin_settings() {
		...
		$active_tab = isset( $_GET['tab'] ) ? sanitize_text_field( $_GET['tab'] ) : 'key';
		?>
	...
	public function register_admin_settings() {
		add_settings_section( 'mmt_migration_key-section', esc_html__( 'Migration Key', 'mmt' ), array( $this, 'migration_key_section_callback' ), $this->admin_slug );
		add_settings_field( 'mmt_migration_key-field', esc_html__( 'Migration Key', 'mmt' ), array( $this, 'migration_key_field_callback' ), $this->admin_slug, 'mmt_migration_key-section' );
		register_setting( $this->admin_slug, 'mmt_migration_key' );
	}

	/**
	 * Migration Key Section Callback
	 *
	 * @since  1.0.0
	 *
	 * @return void
	 */
	public function migration_key_section_callback() {
	 ...
	 * @return void
	 */
	public function migration_key_field_callback( $args ) {
		$migration_key = get_option( 'mmt_migration_key' );
		printf( '<input type="text" name="mmt_migration_key" id="mmt_migration_key" class="mmt_field" value="%s">', esc_attr( $migration_key ) );
	}
	...
	public function register_wizard() {
		if ( empty( $_GET['page'] ) || empty( $_GET['tab'] ) || $this->admin_slug !== $_GET['page'] || 'wizard' !== $_GET['tab'] ) { // Input var ok.
			return;
		}
